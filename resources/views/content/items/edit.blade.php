...
<div class="page-header">
    <h1>Edit item</h1>
    <p class="lead"><a href="{{ route('content.index') }}">← Back to manage content</a> · <a href="{{ route('items.show', $item->slug) }}">View on site</a></p>
</div>
...
<div class="card">
    <form method="post" action="{{ route('content.item.update', $item) }}">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
            ...
            <input type="text" name="slug" id="slug" value="{{ old('slug', $item->slug) }}" required>
        ...
        <div class="form-group">
            ...
            <input type="url" name="source_url" id="source_url" value="{{ old('source_url', $item->source_url) }}">
            @error('source_url')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label for="rarity">Rarity</label>
            <input type="text" name="rarity" id="rarity" value="{{ old('rarity', $item->rarity) }}">
            @error('rarity')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label for="use">Use</label>
            <input type="text" name="use" id="use" value="{{ old('use', $item->use) }}">
            @error('use')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label for="associated_shop">Associated shop</label>
            <input type="text" name="associated_shop" id="associated_shop" value="{{ old('associated_shop', $item->associated_shop) }}">
            @error('associated_shop')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label for="restock_price">Restock price</label>
            <input type="text" name="restock_price" id="restock_price" value="{{ old('restock_price', $item->restock_price) }}">
            @error('restock_price')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label><input type="checkbox" name="is_retired" value="1" {{ old('is_retired', $item->is_retired) ? 'checked' : '' }}> Retired</label>
            @error('is_retired')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label><input type="checkbox" name="is_cavecash" value="1" {{ old('is_cavecash', $item->is_cavecash) ? 'checked' : '' }}> CaveCash</label>
            @error('is_cavecash')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label for="first_appeared">First appeared</label>
            <input type="date" name="first_appeared" id="first_appeared" value="{{ old('first_appeared', $item->first_appeared?->format('Y-m-d')) }}">
