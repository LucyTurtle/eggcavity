    ...
    <form method="post" action="{{ route('content.creature.update', $creature) }}">
        ...
        <div class="form-group">
            ...
            @if($creature->image_url)
                <p style="margin: 0.5rem 0 0 0;"><img src="{{ $creature->image_url }}" alt="" style="max-width: 120px; max-height: 120px; border: 1px solid var(--border); border-radius: var(--radius-sm);" loading="lazy" referrerpolicy="no-referrer"></p>
            @endif
        ...
        <div class="form-group">
            ...
            <input type="text" name="availability" id="availability" value="{{ old('availability', $creature->availability) }}">
            @error('availability')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label for="dates">Dates</label>
            <input type="text" name="dates" id="dates" value="{{ old('dates', $creature->dates) }}">
            @error('dates')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label for="weight">Weight</label>
            <input type="text" name="weight" id="weight" value="{{ old('weight', $creature->weight) }}">
            @error('weight')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label for="length">Length</label>
            <input type="text" name="length" id="length" value="{{ old('length', $creature->length) }}">
            @error('length')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label for="obtained_from">Obtained from</label>
            <input type="text" name="obtained_from" id="obtained_from" value="{{ old('obtained_from', $creature->obtained_from) }}">
            @error('obtained_from')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label for="gender_profile">Gender profile</label>
            <input type="text" name="gender_profile" id="gender_profile" value="{{ old('gender_profile', $creature->gender_profile) }}">
            @error('gender_profile')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label for="habitat">Habitat</label>
            <input type="text" name="habitat" id="habitat" value="{{ old('habitat', $creature->habitat) }}">
            @error('habitat')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label for="about_eggs">About eggs</label>
            <textarea name="about_eggs" id="about_eggs" class="form-group textarea">{{ old('about_eggs', $creature->about_eggs) }}</textarea>
            @error('about_eggs')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label for="about_creature">About creature</label>
            <textarea name="about_creature" id="about_creature" class="form-group textarea">{{ old('about_creature', $creature->about_creature) }}</textarea>
            @error('about_creature')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label for="entry_written_by">Entry written by</label>
            <input type="text" name="entry_written_by" id="entry_written_by" value="{{ old('entry_written_by', $creature->entry_written_by) }}" placeholder="Username">
        ...
        <div class="form-group">
            ...
            <input type="text" name="design_concept_user" id="design_concept_user" value="{{ old('design_concept_user', $creature->design_concept_user) }}" placeholder="Username">
        ...
        <div class="form-group">
            ...
            <input type="text" name="cdwc_entry_by" id="cdwc_entry_by" value="{{ old('cdwc_entry_by', $creature->cdwc_entry_by) }}" placeholder="Username">
            @error('cdwc_entry_by')<small style="color: #dc2626;">{{ $message }}</small>@enderror
        </div>
        <div class="form-group">
            <label for="tags">Tags (comma-separated)</label>
            <input type="text" name="tags" id="tags" value="{{ old('tags', $creature->tags ? (is_array($creature->tags) ? implode(', ', $creature->tags) : (string) $creature->tags) : '') }}" placeholder="tag1, tag2, tag3">
...
<div class="card" style="margin-top: 1.5rem; border-color: var(--border);">
    ...
    <form method="post" action="{{ route('content.creature.destroy', $creature) }}" style="display: inline;" onsubmit="return confirm('Permanently delete this creature? This cannot be undone.');">
        @csrf
        @method('DELETE')
        <button type="submit" class="btn-submit" style="background: #dc2626;">Delete creature</button>
    </form>
</div>
@endsection
